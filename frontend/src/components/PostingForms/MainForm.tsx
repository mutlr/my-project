...
const MainForm = (props: MainFormProps) => {
  ...
  return (
    <div className="post-form-main-container">
      {chosenSong ? (
        <Song
          name={chosenSong.song.songName}
          artist={chosenSong.artist.artistName}
          imageURL={chosenSong.imageUrl}
        />
      ) : (
        <p className={`${getFieldState("song").error ? "postform-error" : ""}`}>
          Choose a song
        </p>
      )}
      ...
      <form className="post-form" onSubmit={handleSubmit(sendData)}>
        ...
        <CustomInput
          register={register}
          name="song"
          placeholder="Type in 3 letters for search to start"
          errors={errors}
        />
        <div>

...
describe('Frontend Tests', () => {
  ...
  describe('Not logged in user view', () => {
    ...
    it('Post and comment button not visible', () => {
      cy.get('.frontpage-container').should('not.contain', 'Add a post');
      cy.get('.frontpage-container').should('contain', TITLE).click();
      cy.get('.postpage-container').should('contain', TITLE);
      cy.get('.postpage-container').should('contain', COMMENT_TITLE);
  ...
  describe('Logged in user', () => {
    ...
    it('Posts show in profile', () => {
      ...
      cy.get('.profile-content-container').should('contain', TITLE);
      cy.get('.profile-content-container').should('not.contain', COMMENT_TITLE);
    ...
    it('Comments show in profile', () => {
      ...
      cy.get('.profile-content-container').should('contain', COMMENT_TITLE);
      cy.get('.profile-content-container').should('not.contain', TITLE);
      cy.get('.profile-content').within(() => {
        cy.get('button').first().contains('Delete');
        cy.get('button').first().next().contains('Edit');
      });
    ...
    it('Can delete comment', () => {
      ...
      cy.visit('/');
      cy.get('.frontpage-container').should('contain', TITLE).click();
      cy.get('.postpage-container').should('not.contain', COMMENT_TITLE);
    });
    it('Can delete post', () => {
      cy.contains('Profile').click();
      cy.get('.filter-container').contains('Posts');
      ...
      cy.get('.profile-content-container').should('not.contain', TITLE);
      cy.visit('/');
      cy.get('.frontpage-container').should('not.contain', TITLE);
    });
  });
});